...
function Dashboard() {
  ...
  if (carregando) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: 'var(--ff-muted)' }}>
        Carregando...
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: 'var(--ff-muted)' }}>
          Sua sessão de hoje aguarda. Vamos estudar!
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Horas hoje', valor: horasHoje, cor: 'var(--ff-green)', badge: `${sessoesHoje.length} sessões`, badgeCor: 'ff-badge-green' },
          { label: 'Total de horas', valor: totalHoras, cor: 'var(--ff-amber)', badge: 'acumulado', badgeCor: 'ff-badge-amber' },
          { label: 'Pomodoros hoje', valor: pomodorosFeitos, cor: 'var(--ff-text)', badge: '25 min cada', badgeCor: 'ff-badge-green' },
          { label: 'Matérias', valor: materias.length, cor: 'var(--ff-accent)', badge: 'cadastradas', badgeCor: 'ff-badge-red' },
        ].map((m, i) => (
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(83,52,131,0.2), rgba(233,69,96,0.1))', border: '1px solid rgba(83,52,131,0.4)', borderRadius: 12, padding: 16 }}>
        ...
        <p style={{ fontSize: 13.5, lineHeight: 1.6, color: 'var(--ff-text)', marginBottom: 12 }}>
          {materias.length === 0
            ? 'Cadastre suas matérias na sidebar para receber sugestões personalizadas de estudo!'
            : sessoesHoje.length === 0
            ? `Você ainda não estudou hoje. Que tal começar com ${materias[0]?.nome}? Uma sessão de 25 minutos já faz diferença!`
            : `Você já estudou ${horasHoje}h hoje. Continue assim! Que tal revisar ${materias[materias.length - 1]?.nome}?`}
        </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={{ background: 'var(--ff-surface)', border: '1px solid var(--ff-border)', borderRadius: 12, padding: 20 }}>
          ...
          {materias.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--ff-muted)' }}>Cadastre matérias para ver o gráfico.</p>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, height: 100 }}>
              {materias.map((m, i) => {
                const min = sessoes.filter((s) => s.materiaId === m.id).reduce((acc, s) => acc + (s.duracaoMinutos || 0), 0);
                const horas = min / 60;
                const pct = maxHoras > 0 ? (horas / maxHoras) * 100 : 5;
                return (
