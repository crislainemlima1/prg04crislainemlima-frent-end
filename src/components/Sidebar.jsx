    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          Tem certeza que deseja remover <strong style={{ color: 'var(--ff-text)' }}>{nomeMateria}</strong>?
...
function Sidebar() {
  ...
  const [confirmarRemover, setConfirmarRemover] = useState(null); // { id, nome }
  ...
  async function confirmarDeletar() {
    try {
      await fetch(`/api/materias/${confirmarRemover.id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      buscarMaterias();
  ...
  return (
    <>
      {confirmarRemover && (
        <ModalConfirmar
          nomeMateria={confirmarRemover.nome}
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          <NavLink to="/painel" end className={({ isActive }) => 'sidebar-item' + (isActive ? ' active' : '')}>
            <span>◈</span> Dashboard
          </NavLink>
          <NavLink to="/painel/timer" className={({ isActive }) => 'sidebar-item' + (isActive ? ' active' : '')}>
            <span>◷</span> Timer Pomodoro
          </NavLink>
          <NavLink to="/painel/resumo" className={({ isActive }) => 'sidebar-item' + (isActive ? ' active' : '')}>
            <span>✦</span> Resumo com IA
          </NavLink>
          <NavLink to="/painel/progresso" className={({ isActive }) => 'sidebar-item' + (isActive ? ' active' : '')}>
            <span>◉</span> Progresso
          </NavLink>
        </nav>
        ...
        <div className="sidebar-secao-header">
          ...
          <button className="sidebar-btn-add" onClick={() => setMostrarForm((v) => !v)} title="Adicionar matéria">
            +
          </button>
        </div>
        ...
        {mostrarForm && (
          <form onSubmit={adicionarMateria} className="sidebar-form">
            <input
              ...
              autoFocus
            />
            <button type="submit" className="sidebar-btn-salvar" disabled={adicionando}>
              {adicionando ? '...' : 'Salvar'}
            </button>
          </form>
        )}
          ...
          {materias.map((m, i) => (
            <div key={m.id} className="sidebar-materia">
              ...
              <button
                ...
                onClick={() => setConfirmarRemover({ id: m.id, nome: m.nome })}
                title="Remover"
              >
                ×
              </button>
        ...
        <div className="sidebar-bottom">
          ...
          >
            ...
            <span style={{ color: 'var(--ff-muted)', fontSize: 12 }}>
              {menuAberto ? '▲' : '▼'}
            </span>
          </div>
          ...
          {menuAberto && (
            <div style={{
              ...
              <button
                onClick={() => { setMenuAberto(false); navigate('/painel/perfil'); }}
                style={{
                  width: '100%', padding: '10px 16px', background: 'none',
                  border: 'none', color: 'var(--ff-text)', fontSize: 13,
              ...
              <button
                ...
                style={{
                  width: '100%', padding: '10px 16px', background: 'none',
                  border: 'none', borderTop: '1px solid var(--ff-border)',
                  color: 'var(--ff-accent)', fontSize: 13, textAlign: 'left',
